...
public class RedisServiceImpl implements IRedisService {
    ...
    public long increment(String redisKey){
        ValueOperations<Serializable, Object> operations = redisTemplate.opsForValue();
        long id = operations.increment(redisKey,1);
        if (id == 1L){
            Calendar cal = Calendar.getInstance();
            cal.set(cal.get(Calendar.YEAR),cal.get(Calendar.MONTH),cal.get(Calendar.DAY_OF_MONTH),
                    23, 59,59);
            cal.set(Calendar.MILLISECOND,999);
            redisTemplate.expireAt(redisKey,cal.getTime());
        }
        return id;
    }

    @Override
    public long cusCode(String key) {
        ValueOperations<Serializable, Object> operations = redisTemplate.opsForValue();
        long id = operations.increment(key,1L);
        if (id==1L){
            id= operations.increment(key,310000L);
        }
        return id;
    }
}
