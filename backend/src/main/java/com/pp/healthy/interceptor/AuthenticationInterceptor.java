...
import com.pp.healthy.annotation.PassToken;
...
//                AdminUserDomain cacheUser = userService.getUserInfoById(userId);
//                if (cacheUser == null) {
//                    throw new RuntimeException("用户不存在，请重新登录");
//                }

//                AdminUserDomain cacheUser = (AdminUserDomain) obj;
                AdminUserDomain cacheUser =  JSONObject.parseObject(obj.toString(), AdminUserDomain.class);
                ...
                // 验证 token
                JWTVerifier jwtVerifier = JWT.require(Algorithm.HMAC256(cacheUser.getPasswork())).build();
                try {
                    jwtVerifier.verify(token);

                    // TODO 先放到session中
                    httpServletRequest.getSession().setAttribute(WebConst.LOGIN_SESSION_KEY, cacheUser);
                    // session 12个小时
